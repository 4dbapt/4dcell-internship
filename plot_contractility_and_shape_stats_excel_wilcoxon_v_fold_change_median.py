...
def log_fold_change_ci(x, y, alpha=0.05):
    ...
    Returns CI for the MEDIAN fold change
    """
...
for sheet_name, df in all_sheets.items():
    ...
    for c in [1, 2, 3, 4]:
        ...
        # ---- FOLD CHANGE MÉDIAN ET IC ----
        # Calculer le fold change médian
        fold_changes = y / x
        median_fold_change = np.median(fold_changes)
        
        # IC sur log scale puis back-transform
        ci_low_fc, ci_high_fc = log_fold_change_ci(x, y)
        
        print(f"    - Fold Change médian: {median_fold_change:.4f}")
        ...
        # Pour le graphique
        medians.append(median_fold_change)
    ...
    if not all(np.isnan(medians)):
        ...
        plt.ylabel(f"Fold Change médian {sheet_name}\n(vs Conc_-1)", fontsize=12, fontweight='bold')

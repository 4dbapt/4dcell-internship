...
# ============================================================================
# STEP 2: NORMALIZE BY DMSO (Second Code)
# ============================================================================
...
def normalize_and_average_contractility(file_path, output_folder=None):
    ...
    exclude_cols = ['Total video time (s)', "Initial Young's modulus", 'Days in culture', 
                   'Well', 'Concentration','Tissue', name_column, RingID,'Resize Factor X', 'Resize Factor Y']
    ...
    return normalized_output, averaged_output

# ============================================================================
# STEP 3: SPLIT BY DRUG (Third Code)
# ============================================================================
...
def step3_split_by_drug(xlsx_path):
    """Step 3: Split normalized XLSX into individual drug XLSX files"""
    print("\n" + "="*80)
    print("STEP 3: SPLITTING NORMALIZED DATA BY DRUG")
    print("="*80)
    ...
    print(f"Found {len(dmso_rows)} DMSO control rows")
...
# ============================================================================
# STEP 4: STATISTICS AND PLOTS (Fourth Code)
# ============================================================================
...
def process_single_normalized_file(file_path):
    ...
    drug_name = Path(file_path).stem.replace('_NormalizedDMSO', '')
...
def main():
    ...
    print("  3. Split normalized data by drug")
    print("  4. Generate statistics and plots")
    print("="*80)
    ...
    try:
        normalized_output, averaged_output = normalize_and_average_contractility(global_csv_path)
    except Exception as e:
    ...
    # STEP 3: Split normalized data by drug
    drug_files = step3_split_by_drug(normalized_output)
    if not drug_files:
    ...
    for drug_file in drug_files:
        try:
            print(f"\n{'='*80}")
            print(f"Processing drug: {drug_file.stem}")
            print(f"{'='*80}")
            
            # Create statistics and plots
            output_path, drug_folder = process_single_normalized_file(str(drug_file))
            
            results.append({
                'drug': drug_file.stem,
                'normalized': drug_file,
                'reorganized': output_path,
                'plots_folder': drug_folder
            })
            
        except Exception as e:
            print(f"❌ ERROR processing {drug_file.name}: {e}")
            import traceback
            traceback.print_exc()
            continue
    ...
    print("="*80)
    print(f"\nProcessed {len(results)} drugs successfully:")
    for result in results:
        print(f"\n  ✓ {result['drug']}:")
        print(f"    - Normalized: {result['normalized']}")
        print(f"    - Reorganized: {result['reorganized']}")
        print(f"    - Plots: {result['plots_folder']}")

...
for sheet_name, df in all_sheets.items():
    ...
    for c in [1, 2, 3, 4]:
        ...
        # Statistique descriptive (médiane uniquement)
        ...
        stats_rows.append([col, "Wilcoxon", stat, pval, median_diff, ci_low, ci_high, n, significance])
    ...
    if stats_rows:
        df_stats = pd.DataFrame(stats_rows, 
                               columns=["Concentration", "Test", "Statistic", "p-value", 
                                       "Median_Diff", "CI_low", "CI_high", "N", "Significance"])
    ...
    if not all(np.isnan(medians)):
        # Utiliser médiane (plus cohérent avec test non-paramétrique)

...
def process_single_normalized_file(file_info):
    ...
    create_plots_and_stats(output_path, drug_name, drug_folder)
    ...
    with pd.ExcelWriter(combined_output, engine='openpyxl') as writer:
        for sheet_name, drug_dfs in all_stats.items():
            ...
            for drug_name, df in drug_dfs:
                ...
                # Add drug name prefix to columns - EXCLUDE Significance column
                drug_prefix = f"{drug_name}_"
                # Only include Mean_FC, SEM_FC, and N columns (not Significance)
...
def main():
    ...
    print("  4. Generate statistics and plots for each drug")
    print("  5. Create combined Wilcoxon stats file")
    print("="*80)
